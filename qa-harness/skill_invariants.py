...
def run(skill_dir: Path) -> Result:
    ...
    if allowed or disallowed:
        ...
        # run of this checker (sort-monash-inbox, 2026-08-09). The two most dangerous tools in that
        ...
        # the leading `\*{0,2}`.
        # The optional non-backticked TAIL before the verb is load-bearing too. Hard Rule 6 of
        # sort-monash-inbox reads "`Write`, `Bash`, ... `Skill` and subagent spawning are removed
        ...
        for m in re.finditer(
            ...
            # `only`, and the sentence preceding sort-monash-inbox's Trash-route bullet happens to end
    ...
    if ref_path.is_file():
        ...
        # § 9", "gmail-ops-playbook ... § 2") points at that document's sections, not this
