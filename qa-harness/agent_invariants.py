...
# The 16 legal frontmatter fields, per ~/.claude/skills/agent-builder/reference.md "all 16".
LEGAL_FIELDS = {
...
PROTOCOL_RULES = [
    {
        ...
        "why": "CLAUDE.md:493 -- '[Needs-info] never counts toward the gate'. An agent that "
    ...
    },
    {
        ...
        "why": "CLAUDE.md:494 -- counts are defects REMAINING after the pass, never intake "
               "counts. Origin (plans/i-want-you-to-dynamic-yeti.md:29): restating intake "
    ...
    },
    {
        ...
        "why": "CLAUDE.md:496 -- 'an agent must never certify its own fixes'. Commit 9daf1dd "
               "records the exact failure: 'the round-3 verifier had proposed the unified "
    ...
    },
    {
        ...
        "why": "prelearn-postlearn.md:574 -- 'Three successive readings of one mutant, three "
...
def check_one(res: Result, path: Path, settings: dict | None):
    ...
    # are a defect of the same kind as a crashing function -- postlearn/SKILL.md once told an
    # agent to "grant each subagent WebSearch and WebFetch and nothing else", which is not a
    # capability that exists (prelearn-postlearn.md:628).
    if tools:
        used = set()
