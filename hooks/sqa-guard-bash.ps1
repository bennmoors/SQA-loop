...
$allowed = @(
    ...
    ("(?!.*\.\.)(?:powershell|pwsh)(?:\.exe)?\s+-NoProfile\s+-NonInteractive\s+-Command\s+Invoke-Pester" +
     ...
     # `Invoke-Pester <temp>/tests -Path C:/Users/example/.claude/hooks` satisfied the row -- the
    ...
    ('(?!.*\.\.)hyperfine(?:\s+(?:(?:-w|--warmup|-m|--min-runs|-M|--max-runs|-r|--runs|-N|--style)\s+\S+|-i|--ignore-failure))*' +
     ...
     #     `hyperfine 'pwsh -File C:/Users/example/evil.ps1'` ran a script from anywhere -- while
