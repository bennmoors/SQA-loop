...
NOISE_DISCLOSURE = [
    ...
    "Cloud-sync clients (OneDrive, Google Drive, ...) watching this tree can steal I/O mid-measurement.",
]
ENERGY_DISCLOSURE = (
...
def guard_command_string(shape: dict) -> str:
    ...
    real documented argv (terms substituted here), `--concepts "Alpha Term; Beta Term;
    Gamma Term"` was split at the semicolons and refused with "'Beta' is not on the
    allowlist (full segment: 'Beta Term')". That made a whole baseline case unmeasurable
    through this instrument while the command itself was perfectly legitimate -- the target
    skill's SKILL.md specifies exactly that `--concepts "<t1; t2; t3>"` shape. shlex.join
    leaves tokens that need no
...
def mode_bench(args, shape) -> int:
    ...
    #
    # Measured 2026-08-21, and this cost an hour before it was understood: every target of
    # one skill pair reads %ONEDRIVE% to locate its data root. Stripped, the root goes
    # relative, the input is not found, the
